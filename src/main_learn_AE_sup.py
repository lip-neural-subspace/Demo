...
def main():
    ...
    pca_basis_dir = None
    if 'pca_basis_dir' in config_json:
        pca_basis_dir = config_json['pca_basis_dir']
    
    cubature_dir = None
    if 'cubature_dir' in config_json:
        cubature_dir = config_json['cubature_dir']
    
    subspace_latent_dim = config_json['subspace_latent_dim']    
    batch_size = config_json['batch_size']
    n_epoch = config_json['n_epoch']
    lr = config_json['lr']
    lr_decay_frac = config_json['lr_decay_frac']
    lr_decay_every = config_json['lr_decay_every']
    lr_warm_up_iters = config_json['lr_warm_up_iters']
    
    # loss weights / style / params
    weight_landscape_reg = None
    if 'weight_landscape_reg' in config_json:
        weight_landscape_reg = config_json['weight_landscape_reg']
        landscape_type = config_json['landscape_type']
    
    evaluation_batch_size = 64
    if 'evaluation_batch_size' in config_json:
        evaluation_batch_size = config_json['evaluation_batch_size']
    
    # Build the system object
    system, system_def = config.construct_system_from_name(config_json['system_name'], config_json['problem_name'])
    full_system_dim = system.dim
    base_pos = system_def['init_pos']
    
    # Build an informative output name
    network_filename_base = f'{args.output_prefix}neural_subspaceAE_' + config_json['system_name'] + '_' + config_json['problem_name'] + f'_dim{subspace_latent_dim}'
    if weight_landscape_reg is not None:
        network_filename_base += f'_landscape{landscape_type}{weight_landscape_reg}'
    ...
    # read pca basis
    if pca_basis_dir is not None:
        print(f'Reading pca basis from {pca_basis_dir}')
        U = igl.read_dmat(os.path.join(pca_basis_dir, "basis.dmat"))
        ae_outer_dim = U.shape[1]

    
    # load cubature datas(if given)
    if cubature_dir:
        indices = igl.read_dmat(os.path.join(cubature_dir, "indices.dmat"))
        weights = igl.read_dmat(os.path.join(cubature_dir, "weights.dmat"))
    ...
    if args.existing_encoder is None:
        ...
        encoder_spec = {
            ...
            'in_dim': ae_outer_dim,
            'hidden_layer_sizes': config_json['encoder_hidden_layers'],
            'out_dim': subspace_latent_dim
        }
    ...
    if args.existing_decoder is None:
        ...
        latent_dim = subspace_latent_dim
        rngkey, subkey = jax.random.split(rngkey)
        decoder_spec = {
            'model_type': args.model_type,
            'activation': args.activation,
            'in_dim': latent_dim,
            'hidden_layer_sizes': config_json['decoder_hidden_layers'],
    ...
    def step_func(i_iter):
        out = lr \
              * jnp.clip((i_iter + 1) / float(lr_warm_up_iters + 1), 0, 1) \
              * (lr_decay_frac ** (i_iter // lr_decay_every))
        return out
    ...
    def batch_loss_fn(params, q_samples):
        ...
        if weight_landscape_reg == 0:
            latent_samples = latent_samples[:evaluation_batch_size]
        ...
        if weight_landscape_reg is not None:
            if weight_landscape_reg > 0:
                ...
                weight_dict['L_landscape'] = weight_landscape_reg
            else:
    ...
    n_batch = math.ceil(n_frame / batch_size)
    save_stride = n_epoch / args.n_save
    ## Main training loop
    for i_epoch in range(n_epoch):
        ...
        for i in range(n_batch):
            ...
            def save_model(this_name, model_params, model_static, model_spec):
                ...
                network_filename_pre = os.path.join(output_dir, network_filename_base) + this_name
                ...
                np.save(
                    network_filename_pre + '_info', {
                        'system': config_json['system_name'],
                        'problem_name': config_json['problem_name'],
                        'latent_dim': subspace_latent_dim,
                        'weight_landscape_reg': weight_landscape_reg
                    })

                print(f'  ...done saving')
        
        def log_statistics():
            print(f'\n== epoch {i_epoch} / {n_epoch}  ({100. * i_epoch / n_epoch:.2f}%) == {time.ctime()}')
            ...
            np.save(os.path.join(output_dir, network_filename_base) + '_history', stats_term_sums_history)
            time_history.append(time.ctime())
            np.save(os.path.join(output_dir, network_filename_base) + '_time_history', time_history)
